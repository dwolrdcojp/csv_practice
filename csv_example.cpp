...
{
	// Make a CSV file with one or more columns of integer calues 
	...
	for(int j = 0; j < dataset.size(); ++j) 
	{
		...
		if(j != dataset.size() - 1) myFile << ","; // No comma at the end of line
	}
	myFile << "\n"; 
	...
	for(int i =0; i < dataset.at(0).second.size(); ++i)
	{
		for(int j = 0; j < dataset.size(); ++j)
		{
			...
			if(j != dataset.size() - 1) myFile << ","; // No comma at the end of the line
		}
		myFile << "\n";
	}
...
std::vector<std::pair<std::string, std::vector<int>>> read_csv(std::string filename)
{
	...
	while(std::getline(myFile, line))
	{
		...
		while(ss >> val) 
		{
			...
			// If the next toke is a comma, ignore it and move on 
